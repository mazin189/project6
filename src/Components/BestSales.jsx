...
import "../App.css"
import {bestsales} from "../Components/AllProducts.js"
...
<p className='text-4xl font-semibold pb-10 pt-16 text-center'>Best Sales</p>
...
<div className='container mx-auto w-full max-w-5xl px-10 md:px-0'>
   <div className='md:grid flex flex-col md:grid-cols-3 gap-x-5 gap-y-6'>



  {bestsales.slice(0,6).map((product,index) => (
    <div key={index} className='bg-white border-2 flex flex-col border-blue-50 rounded-2xl group hover:scale-104 transition duration-700 md:w-82 md:h-100'>
  ...
  <div className="col-span-3 flex flex-col md:flex-row justify-center gap-5">
    {bestsales.slice(6).map((product,index) => (
      <div key={index} className='bg-white flex flex-col border-2 border-blue-50 rounded-2xl group hover:scale-104 transition duration-700 md:w-82 md:h-100'>
        <div className='flex items-end justify-end px-3 pt-4'>
          <FaRegHeart className='text-[18px] opacity-0 group-hover:opacity-100 duration-500 transition cursor-pointer'/>
        </div>
        <img onClick={()=> navigate(`/shop/${product.id}`)} src={product.img} alt={product.alt} className='object-contain w-full max-w-60 h-60 mx-auto cursor-pointer'/>
